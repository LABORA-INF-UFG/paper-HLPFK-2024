...
def solve_model():
    ...
    for u in range(1, config.us+1):
        for t in range(1, config.ti+1):
            ul_as = results['users'][u]['t_' + str(t)]['Uplink']
            dl_as = results['users'][u]['t_' + str(t)]['Downlink']
            bip_v = dict_BIPs['t' + str(t) + '_u' + str(u) + '_bsUL' + str(ul_as) + '_bsDL' + str(dl_as)]
            results['users'][u]['t_' + str(t)]['BIP'] = bip_v
            #print(t)
    
    with open(config.results_file_opt, 'w') as json_file:
