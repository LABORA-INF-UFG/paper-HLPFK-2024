...
def record_json_with_results(solution, data):
    ...
    for item in users:
        ...
        for item2 in range(1, config.ti+1):
            results['users'][item]['t_' + str(item2)] = {'Uplink':None, 'Downlink':None, 'BIP':None}
    ...
    for item in solution:
        ...
        results['users'][int(u)]['t_' + t]['Uplink'] = int(ul)
        results['users'][int(u)]['t_' + t]['Downlink'] = int(dl)
        results['users'][int(u)]['t_' + t]['BIP'] = data['t' + str(t) + '_u' + str(u) + '_bsUL' + str(ul) + '_bsDL' + str(dl)]
        
    with open(config.results_file_meta, 'w') as json_file:
...
if __name__ == '__main__':
    ...
    record_json_with_results(final_solution, data)
    ...
    with open('outputs/metaheuristica.txt', 'a') as file:
