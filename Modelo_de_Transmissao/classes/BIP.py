...
class BIP:
    ...
    def __init__(self):
        self.l_it = self.simulateTransmission()
        self.m_it = self.generateImportancePixelVector()
        #print(self.l_it, self.l_it.shape)
        print('n, l: ', self.m_it, self.m_it.sum(), self.m_it.shape)
    ...
    def calculateUploadDataRate(self, user, base_station, time, users):
        ...
        ijtUL = (config.FUL/4) * math.log2(1 + ((config.Pu * gij * pow(dij, -config.b))/skU))
    ...
    def calculateBipByTransmission(self, user, bs_ul, bs_dl, time, users):
        ...
        print('DataRate: ', c_ijtUL, c_ijtDL)
        
        # Change for Augmented Reality with uplink video transmission
        timeUlTransmission = (config.A) / c_ijtUL
        timeDlTransmission = (2*(self.l_it.sum()/config.BaseResolutionBitsPerPixel/1000)) / c_ijtDL

        delay = timeUlTransmission + timeDlTransmission > config.gammaDelay
        print('DELAY: ', timeUlTransmission, timeDlTransmission)
        quality = np.dot(self.l_it, self.m_it.T) < config.gammaQuality
        print('QUALITY: ', np.dot(self.l_it, self.m_it.T), config.BaseResolution, config.gammaQuality)
        w_it = delay or quality
        print('BIP: ', w_it, delay, quality)
        #w_it = delay
    ...
    def calculateBipTotal(self, user, bs_ul, bs_dl, time, users):
        ...
        print('User Location: ', user.x[1], user.y[1], user.orientation[1], w_it)
        ...
        #print(config.GA, w_it, GA_wit, e_i, e_ga_i, e_B)
        print('SUM:  ', sum)
    ...
    def weightedByNormalization(self, RGBO, RGBC):
        ...
        values = np.concatenate((RNormalized.flatten(), GNormalized.flatten(), BNormalized.flatten()))
